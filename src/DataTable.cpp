...
void DataTable::init_table_info()
{
    ...
    for (rows_count = 0; sw->step(); rows_count++) {
        ...
        temp.sp += sw->getColText(3);
        temp.aboniment_number += sw->getColText(4);
        temp.trener += sw->getColText(5);
...
void DataTable::set_col(int COL, int ROW, int X, int Y, int W, int H)
{
    ...
    case fullname_col:
        ...
        break;
    case sp_col:
        DrawData(info_rows[ROW].sp.c_str(), X, Y, W, H);
        break;
    case aboniment_col:
        DrawData(info_rows[ROW].aboniment_number.c_str(), X, Y, W, H);
        break;
    case trener_col:
        DrawData(info_rows[ROW].trener.c_str(), X, Y, W, H);
        break;
    }
}
...
int DataTable::set_col_w(int COL) const
{
    ...
    case fullname_col:
        return static_cast<int>(w() * 0.298);
    case sp_col:
        return static_cast<int>(w() * 0.3);
    case aboniment_col:
