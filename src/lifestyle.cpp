...
int main()
{
    ...
    try {
        sw = new SQLiteWrapper(DB_DIR, "LIFESTYLE");

        Fl_Window* win = new Fl_Window(win_x, win_y, win_w, win_h, "LifeStyle");
        ...
        TableInputSort* inp_sp
            = new TableInputSort(*table, mid_button_w + spacing_x * 2, 0,
                                 mid_big_button_w, button_h, "Тренер:", 3);

        TableInputSort* inp_aboniment_n = new TableInputSort(
            *table, big_button_w + spacing_x * 2, button_h * 2,
            small_buttow_w - 50, button_h, "Aбонимент:", 4);

        TableInputSort* inp_flat = new TableInputSort(
            *table, big_button_w + spacing_x * 3 + small_buttow_w - 50,
            button_h * 2, small_buttow_w - 50, button_h, "Спорт пит: ", 5);

        std::vector<Fl_Input*> inp_vec
            = {inp_p_number, inp_full_name, inp_sp, inp_aboniment_n, inp_flat};
